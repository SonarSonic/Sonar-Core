...
public class BlockRenderRegister {
	public static void register() {
		...
		for (Block block : SonarBlocks.registeredBlocks) {
			...
			if (item.getHasSubtypes()) {				
				...
				for (ItemStack stack : stacks) {
					...
					ModelLoader.setCustomModelResourceLocation(item, stack.getItemDamage(), new ModelResourceLocation(SonarCore.modid + ":" + item.getUnlocalizedName().substring(5), variant));
				}
			} else {
				registerBlock(block);
			}
	...
	public static void registerBlock(Block block) {
		if (block != null) {
			Item item = Item.getItemFromBlock(block);
			ModelLoader.setCustomModelResourceLocation(item, 0, new ModelResourceLocation(SonarCore.modid + ":" + item.getUnlocalizedName().substring(5), "inventory"));
		}
	}
}
