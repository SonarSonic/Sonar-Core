...
public class ChargingUtils {
	...
	public static boolean canDischarge(ItemStack energy,
	...
	public static EnergyCharge discharge(ItemStack energy, EnergyStorage energyStorage) {
		...
		if (energy != null && stored < max) {
			...
			} else if (SonarAPI.ic2Loaded()
					&& energy.getItem() instanceof IElectricItem) {
		...
		if (energy != null && stored > 0) {
			if (energy.getItem() instanceof IEnergyContainerItem) {
				...
				int itemEnergy = (int) Math.round(Math.min(
						Math.sqrt(item.getMaxEnergyStored(energy)),
						item.getMaxEnergyStored(energy)	- item.getEnergyStored(energy)));
				...
				int itemEnergyRF = (int) Math.round(Math.min(
						Math.sqrt(item.getMaxCharge(energy) * 4),
						(item.getMaxCharge(energy) * 4)	- (manager.getCharge(energy) * 4)));
				int toTransferRF = Math.min(maxTransfer, Math.round(Math.min(itemEnergyRF, stored)));

				energyStorage.setEnergyStored((int) (stored - (manager.charge(
						energy, toTransferRF / 4, 4, false, false) * 4)));
				return new EnergyCharge(-(int) (manager.charge(energy, toTransferRF / 4, 4, false, false) * 4), energy, false);
			} else if (SonarAPI.ic2Loaded()
					&& energy.getItem() instanceof ISpecialElectricItem) {
				...
				return new EnergyCharge(- (int)  (manager.charge(
						energy, toTransferRF / 4, 4, false, false) * 4), energy, false);
			}
		}
		return new EnergyCharge(0, energy, false);
